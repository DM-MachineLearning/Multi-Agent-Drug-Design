...
# Add root directory to path if running as script
if __name__ == "__main__":
...
# Default checkpoint path (Update this to your actual best model path)
DEFAULT_CHECKPOINT = "./vae_ckpts_a40_npy_bidirec/trained_vae_350/vae_weights.pt"
VOCAB_PATH = "vocab.json" # Assumes vocab is in root

def generate(checkpoint_path: str = DEFAULT_CHECKPOINT, vocab_path: str = VOCAB_PATH, num_molecules: int = 20, temperature: float = 0.8) -> list:
    """
    Generate molecular SMILES strings using a trained VAE model.

    Args:
        checkpoint_path (str): Path to the .pt model weights file.
    ...
    # Note: We pass the weights path directly to init based on your VAE class structure
    vae_manager = VAE(model_path=checkpoint_path)

    # 2. Load Model & Vocab
    try:
        # Based on previous fixes, load_model takes 'vocab_base' or similar
        # If vocab_path is full path "vocab.json", usually we just pass that
        if os.path.exists(vocab_path):
            vae_manager.load_model(vocab_base=vocab_path)
        else:
             # Fallback if vocab is in same dir as weights
             parent_dir = str(Path(checkpoint_path).parent)
             vae_manager.load_model(vocab_base=parent_dir)
    ...
    # 3. Generation Loop
    # We loop because your new generate_molecule() generates ONE molecule at a time
    for i in range(num_molecules):
        try:
            # New signature returns tuple: (smiles, z)
            # We also pass exploration_rate if we want variety, or leave default
            smi, _ = vae_manager.generate_molecule()
            ...
            print(f"{i+1:03d}: {status} {smi}")
            
        except Exception as e:
            print(f"{i+1:03d}: ⚠️ Error generating molecule: {e}")

    print(f"\nSummary: {len(valid_molecules)}/{num_molecules} were chemically valid ({len(valid_molecules)/num_molecules:.1%}).")
    return valid_molecules

if __name__ == "__main__":
    # Generate molecules using defaults
    generate(num_molecules=20, temperature=0.8)