...
DATA_PATH = "Models/ActivityClassifier/latent_dataset.pt"
MODEL_PATH = "Models/ActivityClassifier/checkpoints/activity_classifier_mlp.pt"
BATCH_SIZE = 512 # Increased for faster evaluation
...
def evaluate_with_resampling(num_samples=10000, trials=5):
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    print(f"🚀 Evaluating on {device} with {trials} trials...")

    # 1. Load Full Data
    data = torch.load(DATA_PATH)
    full_dataset = TensorDataset(data['z'], data['y'])
    
    if len(full_dataset) < num_samples:
        print(f"⚠️ Dataset size ({len(full_dataset)}) < {num_samples}. Using full dataset.")
    ...
    for trial in range(trials):
        ...
        with torch.no_grad():
            for X_batch, y_batch in loader:
                X_batch = X_batch.to(device)
                probs = model(X_batch)
                preds = (probs > 0.6).float() # Using your 0.6 threshold
                
                y_probs.extend(probs.cpu().numpy())
                y_pred.extend(preds.cpu().numpy())
                y_true.extend(y_batch.numpy())

        # 4. Calculate Metrics for this trial
    ...
    report = pd.DataFrame({
        "Metric": means.index,
        "Mean ± Std Dev": [f"{m:.4f} ± {s:.4f}" for m, s in zip(means, stds)]
    })

    print("\n" + "="*40)
    print(f" ACTIVITY CLASSIFIER BOOTSTRAP REPORT ")
    print(f" (Samples per trial: {num_samples}) ")
    print("="*40)
    print(report.to_string(index=False))

if __name__ == "__main__":
    evaluate_with_resampling(num_samples=500, trials=10)